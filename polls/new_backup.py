...
def vote(request, question_id, maps_id):
    ...
    try:
        ...
        if (Question.objects.filter(pk=question_id+1).exists() == False):
            ...
            for q in Maps.objects.get(pk=maps_id).question_set.all()[:question_id]:
                if (q.pk > question_id):
                    ...
                    question_id = q.pk
                    break
            return HttpResponseRedirect(reverse('polls:detail', kwargs={'question_id':question.id+1, 'maps_id':maps.id,}))
